...
<script>
export default {
  name: 'LukHeritageMetaverseOld'
}
</script>

<script setup>
import {
  createLukHeritageMetaverseOld,
  deleteLukHeritageMetaverseOld,
  getLukHeritageMetaverseOldList
} from '@/api/lukHeritageMetaverseOld'
...
const getTableData = async() => {
  const table = await getLukHeritageMetaverseOldList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteLukHeritageMetaverseOldFunc(row)
        })
    }
...
// 删除行
const deleteLukHeritageMetaverseOldFunc = async (row) => {
    const res = await deleteLukHeritageMetaverseOld({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createLukHeritageMetaverseOld(formData.value)
                  break
              }
              if (res.code === 0) {
                ElMessage({
