...
<script>
export default {
  name: 'LukRevenueHeritage'
}
</script>

<script setup>
import {
  getLukRevenueHeritageList
} from '@/api/lukRevenueNft'
import {
  getLukConstant,
} from '@/api/lukRevenue'
// 全量引入格式化工具 请按需保留
...
const getTableData = async() => {
  const table = await getLukRevenueHeritageList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
