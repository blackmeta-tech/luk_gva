    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="来源" prop="source">
          <template #default="scope">
            <span v-show="scope.row.category == 2 || scope.row.category == 3">购买ID[{{ scope.row.source }}]</span>
          </template>
        </el-table-column>
...
<script>
export default {
  name: 'LukRevenueDetailed'
}
</script>

<script setup>
import { getLukConstant } from '@/api/lukRevenue'
import { getLukRevenueDetailedList } from '@/api/lukRevenueDetailed'
...
const getTableData = async() => {
  const table = await getLukRevenueDetailedList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
