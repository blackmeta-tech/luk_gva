<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="释放总余额" prop="balance" width="120" />
        <el-table-column align="left" label="剩余可释放额度" prop="remaining" width="150" />
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="100px">
        ...
        <el-form-item label="释放总额:"  prop="balance" >
...
const getTableData = async() => {
  const table = await getLukRebateList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
// 删除行
const deleteLukRebateFunc = async (row) => {
    const res = await deleteLukRebate({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createLukRebate(formData.value)
                  break
                default:
                  res = await createLukRebate(formData.value)
                  break
